...
model.load_state_dict(torch.load('Models3d/model3_3d_lr=1e-4_3conv.pth'))
...
total_batch_number = int(np.ceil(training_ids.shape[0] / params['batch_size']))

print('Training starts!')

while epoch < total_epochs and val_loss_tracker < 30:
  epoch_loss = 0
  ...
  with torch.no_grad():
    for volume, tensor, interpolation in validation_generator:
      ...
      if val_loss < lowest_val_loss:
        ...
        val_loss_tracker = 0
        torch.save(model.state_dict(), 'Models3d/model3_3d_1_lr=1e-4_3conv.pth')
    
  epoch += 1
  val_loss_tracker += 1
  print(f"Training loss at epoch {epoch} is {epoch_loss / total_batch_number} and validation_loss is {val_loss}")
